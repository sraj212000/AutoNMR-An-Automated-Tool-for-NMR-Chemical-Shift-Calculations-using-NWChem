...
def generate_3d_coordinates(smiles, sdf_data, xc_functional, output_file_prefix, case):
    ...
    Args:
        ...
        case (int): Case selection for shielding tensor calculation (1 for Hydrogen, 2 for Carbon).
...
def run(smiles):
    ...
    Global Variables:
        ...
        output_file_name (str): The name of the NWChem output file.
...
def main():
    ...
    parser.add_argument("reference_shielding", type=str, help="Reference shielding values in the format 'H:32.8684825,C:190.9843647'.")
    ...
    reference_shielding = {item.split(':')[0]: float(item.split(':')[1]) for item in args.reference_shielding.split(',')}
    ...
    write_chemical_shifts_to_excel(output_dir, reference_shielding, args.output_excel)
